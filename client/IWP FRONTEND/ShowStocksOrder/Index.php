...
<H2 style="text-align:center;"><strong><mark><blink>Pending Payments</blink></mark></strong></H2><Br><Br>


<?php
$fetchQuery = mysqli_query($conn,"select * from bb_suppliers") or die("could not fetch".mysqli_error($conn));
?>
...
<?php
	if(isset($_POST['submitDeleteBtn']))
	{
		...
		$check= mysqli_query($conn,"select * from bb_suppliers where order_id='$key'") or die("not found".mysqli_error($conn));
		if(mysqli_num_rows($check)>0)
		{
			$queryDelete= mysqli_query($conn,"Delete from bb_suppliers where order_id='$key' ")
		...
		{?>
			<tr>
				<form action="" method="post" role="from">
					...
					<td><?php echo $row['dd_cheque_no'];?></td>
					<td><?php echo $row['drawn_on'];?></td>
					<td><?php echo $row['payable_at'];?></td>
					<td>
						<input type="checkbox" name="keyToDelete" value="<?php echo $row['order_id'];?>" required>
					</td>
					<td>
						<input type="Submit" name="submitDeleteBtn" class="btn btn-info" Value="Clear">
					</td>
				</form>
